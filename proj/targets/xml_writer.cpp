...
void til::xml_writer::do_print_node(til::print_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') << "<" << node->label() << " append_newline='"
       << std::boolalpha << node->append_newline() << std::noboolalpha << "'>"
       << std::endl;
...
void til::xml_writer::do_function_definition_node(til::function_definition_node * const node, int lvl) {
  ...
  os() << std::string(lvl, ' ') << "<" << node->label() << " type='"
       << cdk::to_string(node->type()) << "' isMain='"
       << std::noboolalpha << node->isMain() << std::boolalpha << "'>"
       << std::endl;
  if (node->arguments()) {
...
void til::xml_writer::do_with_change_node(til::with_change_node * const node, int lvl) {
  // EMPTY
}